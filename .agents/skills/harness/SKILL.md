...
**核心原则：**
1. 创建 Agent 定义(`.Codex/agents/`)和 Skill(`.Codex/skills/`)。
2. **Agent 团队作为默认执行模式。**
3. **在 AGENTS.md 中注册 Harness 指针。** — 仅记录最少指针（触发规则 + 变更历史），使新会话能触发编排器 Skill。
4. **Harness 不是固定产物，而是持续进化的系统。** — 每次执行后收集反馈，持续更新 Agent、Skill、AGENTS.md。
...
1. 读取 `项目/.Codex/agents/`、`项目/.Codex/skills/`、`项目/AGENTS.md`
2. 根据状态分叉执行模式：
...
3. 对照现有 Agent/Skill 列表与 AGENTS.md 记录，检测不一致（drift）
...
**所有 Agent 必须定义为 `项目/.Codex/agents/{name}.md` 文件。** 禁止不创建 Agent 定义文件而直接将角色写入 Agent 工具的 prompt。原因：
...
在 `项目/.Codex/agents/{name}.md` 中定义每个 Agent。必须章节: 核心角色、工作原则、输入/输出协议、错误处理、协作。Agent 团队模式下添加 `## 团队通信协议` 章节，明示消息收发对象和工作请求范围。
...
在 `项目/.Codex/skills/{name}/SKILL.md` 创建各 Agent 使用的 Skill。详细编写指南参见 `references/skill-writing-guide.md`。
...
description 是 Skill 的唯一触发机制。Codex 倾向于保守判断触发，因此 description 应 **主动（"pushy"）** 编写。
...
#### 5-4. AGENTS.md Harness 指针注册

Harness 配置完成后，在项目 `AGENTS.md` 中注册最少指针。AGENTS.md 每个新会话都加载，仅记录 Harness 存在和触发规则，编排器 Skill 处理其余。
...
**不放入 AGENTS.md 的内容:** Agent 列表、Skill 列表、目录结构、执行规则详情。原因: Agent/Skill 列表由编排器 Skill 和 `.Codex/agents/`、`.Codex/skills/` 管理，属重复。目录结构可直接从文件系统确认。AGENTS.md 仅承载 **指针（触发规则）+ 变更历史**。
...
所有变更记录在 AGENTS.md 的 **变更历史** 表（与 Phase 5-4 模板的"变更历史"节相同表）:
...
**Step 1: 现状审计**
- 对比 `.Codex/agents/` 文件列表与编排器 Skill 的 Agent 配置 → 生成不一致列表
- 对比 `.Codex/skills/` 目录列表与编排器 Skill 的 Skill 配置 → 生成不一致列表
- 向用户报告审计结果
...
- AGENTS.md 与实际文件一致性最终确认

## 产物检查清单

生成完成后确认:

- [ ] `项目/.Codex/agents/` — **Agent 定义文件必须创建**（内置类型也必须创建文件）
- [ ] `项目/.Codex/skills/` — Skill 文件（SKILL.md + references/）
- [ ] 1 个编排器 Skill（包含数据流 + 错误处理 + 测试场景）
- [ ] 明示执行模式（Agent 团队 / Sub Agent / 混合模式 三者选一，混合模式下注明各 Phase 模式）
- [ ] 所有 Agent 调用包含 `model: "opus"` 参数
- [ ] `.Codex/commands/` — 不生成任何内容
- [ ] 与既有 Agent/Skill 无冲突
...
- [ ] **AGENTS.md 中注册 Harness 指针**（触发规则 + 变更历史）
- [ ] **AGENTS.md 变更历史中记录 Agent/Skill 添加/删除/修改**

...
<template>
  ...
  <article v-else class="kd-detail">
    <div class="kd-detail-header"><div class="kd-detail-identity"><span class="kd-detail-index">RECORD / {{ item.id.slice(-4).toUpperCase() }}</span><div class="kd-breadcrumb">{{ item.status === 'archived' ? '归档库' : '知识库' }} / {{ item.tags[0] ?? '未分类' }} / {{ typeCopy[item.type] }}</div></div><div class="kd-inline-actions"><button v-for="action in actions" :key="action.id" :class="`kd-action-button kd-action-button--${action.tone}`" :disabled="actionState?.itemId === item.id && actionState.action === action.id" type="button" @click="void onAction(item, action.id)">{{ actionState?.itemId === item.id && actionState.action === action.id ? '处理中' : action.label }}</button><button class="kd-action-button" type="button" @click="onAskAssistant(item, 'summary')"><MessageCircle :size="15" /> 问云端助手</button><button v-if="canAskBody" class="kd-action-button" type="button" @click="onAskAssistant(item, 'body')"><FileText :size="15" /> 带正文提问</button><button class="kd-action-button" :disabled="editing" type="button" @click="beginEdit"><Pencil :size="15" /> 编辑</button></div></div>

...
class AuthControllerTest {
    ...
    void setUp() {
        ...
        AuthController authController = new AuthController(authService, rateLimiterService, appProperties);
    ...
    void shouldReturnBadRequestWhenPasswordIsWeak() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register")
                ...
                .andExpect(jsonPath("$.message").value("Password must include upper/lowercase letters, digits and special characters"));
    }
    ...
    void shouldReturnTooManyRequestsWhenLimitExceeded() throws Exception {
        ...
        // 模拟 IP 被限流
        when(rateLimiterService.allow(eq("ratelimit:register:ip:127.0.0.1"), eq(10L), any(Duration.class)))
                .thenReturn(false);

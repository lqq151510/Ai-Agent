...
public class StartupValidationRunner implements ApplicationRunner {
    ...
    private void validateRequiredDependencies() {
        validateRequiredDependency(
                "database",
                diagnosticsService::checkDatabase,
                "Database connectivity check failed: ");
        validateRequiredDependency(
                "redis", diagnosticsService::checkRedis, "Redis connectivity check failed: ");
    }
    ...
    private void validateRequiredDependency(
            String dependencyName, Supplier<ReadinessCheck> checkSupplier, String errorPrefix) {
        ReadinessCheck check = checkSupplier.get();
        if (!check.ok()) {
            throw new IllegalStateException(errorPrefix + check.detail());
        }
        log.info(
                "Startup required dependency check passed: {} ({})",
                dependencyName,
                check.detail());
    }
    ...
    private void validateJwtSecret() {
        ...
        if (jwtSecret == null || jwtSecret.isBlank()) {
            throw new IllegalStateException(
                    "JWT_SECRET or security.jwt.secret is required and must be at least 32"
                            + " characters.");
        }
    ...
    private void validateModelConfig() {
        if (appProperties.getDefaultProvider() == ModelProviderType.OPENAI) {
            ...
            if (apiKey == null || apiKey.isBlank()) {
                throw new IllegalStateException(
                        "OPENAI_API_KEY is required when app.default-provider=OPENAI");
            }
            if (baseUrl == null || baseUrl.isBlank()) {
                throw new IllegalStateException(
                        "OPENAI_BASE_URL is required when app.default-provider=OPENAI");
            }
            if (model == null || model.isBlank()) {
                throw new IllegalStateException(
                        "OPENAI_MODEL is required when app.default-provider=OPENAI");
            }
        }
    }
    ...
    private void validateOptionalModelProvider() {
        ...
        if (check.ok()) {
            log.info(
                    "Startup optional dependency check passed: model provider ({})",
                    check.detail());
            return;
        }
        ...
        if (appProperties.getStartupValidation().isFailFast()) {
            throw new IllegalStateException(
                    "Model provider readiness check failed: " + check.detail());
        }

        log.warn(
                "Startup optional dependency check failed but fail-fast disabled: model provider"
                        + " ({})",
                check.detail());
    }
}

...
public class AuthController {
    ...
    public UserProfileResponse register(
        ...
        checkRateLimit(
                ...
                "register",
                request.email(),
                appProperties.getRateLimit().getRegisterPerMinute());
    ...
    public TokenResponse login(
        ...
        checkRateLimit(
                ...
                "login",
                request.email(),
                appProperties.getRateLimit().getLoginPerMinute());
    ...
    public TokenResponse refresh(
        ...
        checkRateLimit(
                httpRequest, "refresh", null, appProperties.getRateLimit().getRefreshPerMinute());
    ...
    public UserProfileResponse updateConfig(
            @Valid @RequestBody com.agent.mvp.auth.dto.UpdateUserConfigRequest configRequest,
    ...
    private void checkRateLimit(
            HttpServletRequest request, String action, String email, long limitPerMinute) {
        String ip = resolveClientIp(request);
        long limit = Math.max(1, limitPerMinute);

        boolean allowedByIp =
                rateLimiterService.allow(
                        "ratelimit:" + action + ":ip:" + ip, limit, Duration.ofMinutes(1));
        if (!allowedByIp) {
            throw new TooManyRequestsException("Too many " + action + " attempts");
        }

        if (email != null) {
            boolean allowedByEmail =
                    rateLimiterService.allow(

...
public class ReleaseReportService {
    ...
    public ReleaseReportService(
            SystemDiagnosticsService diagnosticsService, ToolAuditService toolAuditService) {
    ...
    public String buildMarkdown(UUID userId, int windowHours, UUID sessionId) {
        ...
        out.append("- Session Scope: ")
                .append(report.sessionId() == null ? "global" : report.sessionId())
                .append("\n\n");
        ...
        for (ReadinessCheck check : report.readiness().checks()) {
            out.append("| ")
                    ...
                    .append(check.ok())
                    .append(" | ")
                    .append(safeMd(check.code()))
                    .append(" | ")
                    .append(check.latencyMs() == null ? "" : check.latencyMs())
                    .append(" | ")
                    .append(safeMd(check.detail()))
                    .append(" |\n");
        }
        out.append("\n");
        ...
        out.append("- Avg/P95 (ms): ")
                .append(report.toolStats().averageDurationMs())
                .append(" / ")
                .append(report.toolStats().p95DurationMs())
                .append("\n\n");

...
public class ScaffoldTemplateRegistry {
    ...
    public GeneratedScaffold generate(ScaffoldRequest request) {
        ...
        List<ScaffoldFile> files = List.of(
                new ScaffoldFile("pom.xml", pom(projectName)),
                new ScaffoldFile("README.md", readme(projectName, preset, description)),
                new ScaffoldFile("src/main/resources/application.yml", applicationYaml(projectName)),
                new ScaffoldFile("src/main/java/" + packagePath + "/" + className(projectName) + "Application.java", applicationClass(basePackage, className(projectName))),
                new ScaffoldFile("src/main/java/" + packagePath + "/api/DevCoachController.java", controller(basePackage, preset)),
                new ScaffoldFile("src/main/java/" + packagePath + "/service/DevCoachService.java", service(basePackage, preset)),
                new ScaffoldFile("src/test/java/" + packagePath + "/service/DevCoachServiceTest.java", serviceTest(basePackage))
        );
        ...
        return new GeneratedScaffold(
                ...
                projectName,
                files,
                List.of("mvn -q test", "mvn spring-boot:run")
        );
    }

    public Map<String, String> listPresets() {
        return Map.of(
                "spring-ai-rag-starter", "Spring Boot RAG starter with retrieval-oriented service boundaries",
    ...
    private String defaultDescription(String preset) {
        return switch (preset) {
            case "spring-ai-rag-starter" -> "A Spring Boot starter for RAG-style Java AI applications.";
            case "langchain4j-agent-starter" -> "A Java agent starter with planner and tool execution boundaries.";
    ...
    private String service(String basePackage, String preset) {
        String nextStep = switch (preset) {
            case "spring-ai-rag-starter" -> "Define document ingestion, retrieval interface, and answer synthesis.";
            case "langchain4j-agent-starter" -> "Define planner, tool registry, execution trace, and safety limits.";

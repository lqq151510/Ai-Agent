...
CREATE INDEX IF NOT EXISTS idx_model_sources_user_created ON model_sources(user_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_model_sources_user_name ON model_sources(user_id, name);
CREATE INDEX IF NOT EXISTS idx_knowledge_items_user_status_updated ON knowledge_items(user_id, status, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_knowledge_items_user_source_type ON knowledge_items(user_id, source_type);
CREATE INDEX IF NOT EXISTS idx_knowledge_items_user_created ON knowledge_items(user_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_knowledge_tags_user_name ON knowledge_tags(user_id, name);
CREATE INDEX IF NOT EXISTS idx_knowledge_item_tags_tag_id ON knowledge_item_tags(tag_id);
CREATE INDEX IF NOT EXISTS idx_ingestion_jobs_user_created ON ingestion_jobs(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_ingestion_jobs_item_created ON ingestion_jobs(knowledge_item_id, created_at DESC);

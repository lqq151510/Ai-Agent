...
>
> **使用规则：**`v0.1.0-beta.2` 只证明该 tag 对应的发布物；测试与覆盖率必须同时给出执行日期、命令和源代码边界。2026-08-27 的后端数据来自有未提交改动的本地工作区，尚不是 Beta.2 或任何已发布版本的证明。
...
### 2.1 当前后端开发基线（2026-08-27，非发布证据）

源代码边界：`main` 基于 `2dcce90`，执行时工作区含未提交改动。该记录可用于说明当前开发质量，但在提交、推送并对目标提交复跑前，不能写成已发布 Beta 的测试结果。
...
- `backend`：341 tests run，0 failures，0 errors，14 skipped。
...
以下记录保留为当时的跨运行时验证快照。它们没有绑定不可变提交，不应与 `v0.1.0-beta.2` tag 或本页 2026-08-27 的后端基线混用；对外使用前应在目标提交复跑。
...
本次 `clean verify` 生成的 `backend/target/site/jacoco/jacoco.xml` 汇总：

- Lines：5392 covered / 7152 total，75.4%
- Branches：1612 covered / 2607 total，61.8%
- Methods：999 covered / 1260 total，79.3%
- Classes：255 covered / 281 total，90.7%

覆盖率只描述此次本地 `backend` JaCoCo 统计，不外推为 Electron、Renderer 或整仓覆盖率；也不表示 Beta.2 发布包已经以这些数字复验。
...
- 数字必须带范围、日期和来源边界，例如“2026-08-27 本地开发基线的 backend 341 个测试”，不说“项目共有 341 个测试”。
- 不把有未提交改动的测试或覆盖率归因给 tag、Release 或已发布安装包；提交后仍要在该提交上复跑。
- 历史 225 个测试是 2026-08-20 的验证快照，不能与当前 341 个后端测试相加，也不能默认归因给 Beta.2。

...
- **回答要点**：
  ...
  2. **端到端可靠性设计**：
     - **Producer 端**：`KnowledgeIngestionProducer` 投递消息后执行 `future.get(2, TimeUnit.SECONDS)` 同步等待 Broker ACK 确认；若超时或发生异常，记录警告并即时退避降级为本地直接向量切片入库，杜绝消息静默丢失。
     - **Consumer 端**：`KnowledgeIngestionConsumer` 消费消息并执行切片与向量化入库；当遇到非预期异常时记录告警并**显式重新抛出 `RuntimeException`**，交由 Spring Kafka 容器的 `DefaultErrorHandler` 执行重试与死信投递，而不是被内部静默吞掉。
  3. **防重复（幂等消费）**：消息体携带明确的 `userId` 与 `knowledgeItemId`；切片入库前依据条目状态与内容哈希校验，若已处理直接跳过，保证幂等。

### Q4：Kafka 消费失败如何处理？死信队列（DLT）怎么设计的？
- **回答要点**：
  - 在 Spring Kafka 配置中注册 `DefaultErrorHandler`，搭配 `DeadLetterPublishingRecoverer`。
  - 对于可重试异常（如临时网络抖动、向量存储短暂超时），执行重试；
  - 超过重试次数或遇到无法恢复的错误，消息被安全转移至 `topic-retrieval.DLT` 死信队列，保障主消费链路不被毒丸消息（Poison Pill）阻塞，并支持后续人工排查与重放。
...
- **回答要点**：
  ...
  3. **两级缓存的同步机制**：
     ...
     - 架构推演中可结合 Redis Pub/Sub 或 Canal 监听 Binlog 广播失效消息，各个节点的 JVM 监听到事件后使本地 Caffeine L1 缓存失效。
...
- **回答要点**：
  ...
  4. **容量推演数据说明**：在基准模型推演中，基于 L1 命中率 80%（0.15ms 内存耗时）、L2 命中率 15%（2.1ms 网络耗时）、DB 穿透 5%（18.5ms）的行业典型分布，推演出多级综合读吞吐理论上限可达 **32,000 QPS**，加权理论延迟控制在 **0.78ms**。面试时应明确说明这是架构容量推演模型，体现系统设计与并发容量估算能力。
...
- **回答要点**：
  ...
  3. **基准验证与工程权衡**：在自建评测套件（8 篇典型技术文档、13 组测试查询）中，RRF 混合检索的 Top-3 召回率达 **92.3%**（优于纯关键词的 84.6%），MRR 为 0.8718（优于关键词 0.8327）。虽然因为 \(k=60\) 平滑常数的存在，MRR 略低于纯密集向量的 0.8942，但它彻底解决了纯向量在面对生僻英文专有名词（如 `efConstruction`、`HMAC-SHA256`）时的语义漂移问题，工程鲁棒性更优。
...
- **回答要点**：
  1. **实验数据推导**：
     - 当阈值 < 0.88 时，语义漂移明显，反义或不同场景的问题容易被误识别为同一意图；
     - 当阈值 > 0.95 时，命中率急剧跌至 10% 以下，失去了缓存加速价值；
     - 选定 **0.92** 作为最优平衡点：在蒙特卡洛灵敏度模拟中（假设 65% 目标命中率），命中请求响应约 **25ms**（对比无缓存冷请求约 1350ms），可推演节约 **64.8% 的 Token 支出**。
...
- **回答要点**：
  ...
  - 配置规则（Rule）：
    ...
    ```
  - 当前后端全量运行 **352 项自动化测试**（0 失败、0 错误、9 跳过），实测行覆盖率 **76.09%**（\(\ge 65\%\)），分支覆盖率 **62.17%**（\(\ge 60\%\)），均稳定越过强门禁标准。任何提交若破坏分支覆盖率或测试通过率，本地构建与 GitHub Actions 均会直接失败阻断。

...
- 2026-09-14 更新：**关闭 §5 G3** —— `.github/workflows/ci.yml` 新增独立 `python-backend-test` job（Python 3.12 + `astral-sh/setup-uv` v8.3.2 + `uv sync --frozen --extra dev` + `uv run pytest --cov=knowledge_desk`），旧的 `python-service-test`（Python 3.11）保留；`sentinel-alert.needs` 同步纳入新 job。附录 A 新增 A21 并标注为**部分验证**（workflow 结构与依赖图已本地校验、测试命令已等价实跑 `173 passed`，但尚未在 GitHub Actions 实际运行）。同步清除 `README.md`、`PROJECT_SHOWCASE.md`、`RESUME_PROJECT_GUIDE.md`、`python-backend/README.md` 中「CI 未覆盖 `python-backend/`」的失效表述。
...
| A21 | CI 已覆盖 `python-backend/` 基线（新增 `python-backend-test` job） | 本文 §5 G3；README「CI 覆盖（2026-09-14 校准）」 | `grep -n "python-backend-test" .github/workflows/ci.yml` + 解析 YAML 校验 job 依赖；本机等价：`cd python-backend && .venv/bin/python -m pytest --cov=knowledge_desk` | job 存在，步骤为：Python 3.12 → `astral-sh/setup-uv@11f9893b081a58869d3b5fccaea48c9e9e46f990`（v8.3.2）→ `uv sync --frozen --extra dev` → `uv run pytest --cov=knowledge_desk --cov-report=term-missing`；`sentinel-alert.needs` 已包含该 job；YAML 解析通过、needs 无悬空引用；本机等价复跑 `173 passed` / 91% | ⚠️ **部分验证**：workflow 结构与依赖图已在本地校验、测试命令已按等价方式实跑（173 passed），但该 job **尚未在 GitHub Actions 上实际运行过**；首次触发后必须回填真实运行结论 |

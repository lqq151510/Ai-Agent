...
- Python 基线采用 **ad-hoc / linker 签名**（无需 Apple 开发者账号，本机可直接启动演示）；**不做** Developer ID 签名与 Apple 公证，属个人简历项目的范围决策。**数据闭环已实证**：2026-09-14 用打包运行时同一后端二进制 + 隔离数据目录，跑通「导入 → 整理 → 搜索 → 复习 → 重启后仍存在」共 17 项检查。**真实模型联调已通过**：2026-09-15 用智谱 GLM-4-Flash（免费模型）完成 19 项检查，含真实整理（`note=model`）、本地启发式对照、失败降级与凭据加密。**人工 GUI 窗口交互回归仍未完成**。
...
| 真实模型联调（智谱 GLM-4-Flash） | 隔离环境 + 免费 OpenAI-compatible 端点 | **19 项检查全部 PASS**：连接测试、真实整理产出摘要与语义标签（任务记录 `note=model`）、停用后回落 `local_heuristic`、无效凭据降级、凭据以 `enc:v1:` 落库；日志与数据目录内均无密钥明文 |
...
- 该结果只描述**工程完成度**，不构成安装包发布结论：未构建 x64 与 universal；签名采用 ad-hoc（个人演示范围），不做 Developer ID 签名与公证。数据闭环（17 项）与真实模型联调（19 项，智谱 GLM-4-Flash）均已在隔离环境中验证通过，但**人工 GUI 窗口交互回归**仍未完成。

...
>
> 后端可替换性是我后来加的一条线：在不动 `/api/v1` 契约的前提下，我并排做了一条 FastAPI + SQLite 的 Python 本地后端，桌面包可以同时携带两条基线，用 `backend-runtime.json` 显式选择启动哪一条。这里我刻意**不做隐式回退**——选中的基线缺产物就启动失败并报出缺失清单，因为静默换一条基线会在 H2 和 SQLite 之间悄悄改变数据路径。Python 基线本机 173 项 pytest 通过、arm64 打包运行时验收 9 项全过，并且已用智谱 GLM-4-Flash 免费模型完成真实整理联调（连接测试、真实摘要与标签、停用后回落本地启发式、失败降级、凭据加密）；但它目前只是本机 arm64 的工程完成度，尚未签名、未构建 x64。

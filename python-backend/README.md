...
4. **已完成**（2026-09-14）：**CI 覆盖本目录**——`.github/workflows/ci.yml` 新增独立的 `python-backend-test` job（Python 3.12 + uv），执行 `uv sync --frozen --extra dev` 与 `uv run pytest --cov=knowledge_desk`；原有的 `python-service-test`（Python 3.11，旧的文档解析服务）保留不变。
